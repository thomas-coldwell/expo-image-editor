...
import { editingModeState, glContextState, imageBoundsState, imageDataState, processingState, } from "../Store";
...
export function Blur() {
        ...
        if (gl) {
            ...
            if (Platform.OS === "web") {
                ...
                fileReaderInstance.onload = async () => {
                    ...
                    const flippedOutput = await ImageManinpulator.manipulateAsync(base64data, [{ flip: ImageManinpulator.FlipType.Vertical }]);
                    setImageData({
                ...
                const flippedOutput = await ImageManinpulator.manipulateAsync(output.uri, [{ flip: ImageManinpulator.FlipType.Vertical }]);
                setImageData({
                    uri: flippedOutput.uri,
                    width: flippedOutput.width,
                    height: flippedOutput.height,
    ...
    const throttleSliderBlur = React.useRef(throttle((value) => setBlur(value), 50, { leading: true })).current;
    React.useEffect(() => {
        return () => { };
    });
    if (glContext === null) {
    ...
    return (React.createElement(View, { style: styles.container },
        React.createElement(View, { style: [styles.row, { justifyContent: "center" }] },
            React.createElement(Slider, { value: sliderValue, onValueChange: (value) => {
        ...
        React.createElement(View, { style: styles.row },
            ...
            React.createElement(Text, { style: styles.prompt },
                "Blur Radius: ",
                Math.round(sliderValue)),
            React.createElement(IconButton, { iconID: "check", text: "Done", onPress: () => onSaveWithBlur() }))));
}
