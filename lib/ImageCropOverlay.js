...
function ImageCropOverlay(props) {
    const [selectedFrameSection, setSelectedFrameSection] = react_1.useState('middlemiddle');
    const [panResponderEnabled, setPanResponderEnabled] = react_1.useState(false);
    const { imageBounds, fixedAspectRatio, accumulatedPan, cropSize } = props;
    const pan = react_1.useRef(new react_native_1.Animated.ValueXY({
        x: imageBounds.x,
        y: imageBounds.y
    })).current;
    ...
    const [panResponder, setPanResponder] = react_1.useState(panInstance);
    react_1.useEffect(() => {
