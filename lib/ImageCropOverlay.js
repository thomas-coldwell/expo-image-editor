...
function ImageCropOverlay() {
    ...
    const cropWidthDelta = React.useRef(new react_native_1.Animated.Value(0));
    const cropHeightDelta = React.useRef(new react_native_1.Animated.Value(0));
    React.useEffect(() => {
    ...
    const getTargetCropFrameBounds = ({ nativeEvent, }) => {
        let x = 0;
        let y = 0;
        let width = 0;
        let height = 0;
